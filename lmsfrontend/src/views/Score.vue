<template>
    <div class="skill">
        <div class="sidebar">
            <Sidebar></Sidebar>
        </div>
        <div class="dashnav">
          <Dashboardnavbar></Dashboardnavbar>
        </div>
		<div class="skillpage">
			<Section class=" quiz-cards">
				<div class='quiz-card card col'>
					<div class='quiz-data '>
						
						<div class='quiz-info col-6'>
							<h2 class="card-title">Your Score</h2>
							
						<router-link tag="button"  :to="{ name:'Question', params: {slug: 'business-analysis'} }" exact type="button" class='btn quiz-btn' data-toggle='modal' data-target='#exampleModal'>
							Retake Quiz
						</router-link>
						</div>
					</div>
				</div>
				
				
			</Section>
		</div>
        
    </div>    
</template>

<script>
import Sidebar from '@/components/Dashboard/Sidebar.vue'
import Dashboardnavbar from '@/components/Dashboard/Dashboardnavbar.vue'

export default {
// @ is an alias to /src
  name: 'Dashboard',

  components:{
    Sidebar,
    Dashboardnavbar,
  },
}
</script>

<style scoped>
.sidebar{
  position: fixed;
  z-index: 2;
}

.quiz-info-list > li{
  list-style: none;
}

.dashnav{
  position: static;
  z-index: 1;
}

.quiz-card{
	width: 50rem !important;
	margin-bottom: 3rem;
    margin-left: 13rem;
}

.quiz-info, .quiz-img{
  padding: 0;
  position: unset;
}

.responsive {
  max-width: 100%;
  height: 100%;
}


.quiz-btn{
	border-radius: 0px;
  position: absolute;
  right: 0;
  bottom: 0;
}

.quiz-context{
	height: 80%;
}

.modal-data{
height:450px;
width:450px;
font:16px/26px Lato, Garamond, Serif;
overflow:auto;
}

.skillpage{
  position: absolute;
  padding-top: 3rem;
  margin: 0 auto;
  height: 100%;
}

Section{
    width: 100%;
    /* display: inline-block; */
    float: left;
    margin: 0 auto;
    padding:50px 0 0 100px;
    text-justify: auto;
}

@import url('https://fonts.googleapis.com/css2?family=Lato:ital,wght@0,300;0,400;0,700;0,900;1,300;1,700&display=swap');


html{
    font-size: 62.5%;
    /* font-family: 'Lato', sans-serif; */
}

.startmodal{
    width: 50vw;
    height: 70vh;
    /* margin: auto; */
    position: absolute;
    left: 50%;
    top: 50%;
    transform: translate(-50%, -50%);
    font-size: 1.6rem;
}

.header{
    text-align: center;
}

.info-header{
    font-weight: 600;
    line-height: 1.75;
    margin-bottom: 8px !important;
    color: rgba(0,0,0,.9);
}

.info-text{
    line-height: 2.5;
}

.overview_list{
    list-style-type: none;
    /* font-size: 0; */
}

/* li{
    margin-bottom: 1.5rem;
} */
.quiz-card{
    height: 20rem;
    width: 50px;
    position: center;
    border-radius: 10px;
    border-color: white;
    background-color: rgb(220, 227, 231)
}
.btn{
    background-color: rgb(142, 223, 121); 
    text-decoration: none;
    text-decoration: none;
    margin-right: 2rem;
    color: #fff;
         

}
</style>