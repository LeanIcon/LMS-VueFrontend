<template>
  <div class="forgotpassword">
    ...
    <div class="page__container row">
      <a class="return_btn" onclick="history.back()"
        ><i class="back--btn fa fa-arrow-left"></i> Back</a
      >
        ...
        <div class="display_content row">
          ...
          <div class="col-lg-8 col-md-12 info_section">
            <form action="" method="POST" v-on:submit.prevent="register">
              ...
              <button class="btn btn-primary" v-on:submit.prevent="register">
                Register
              </button>
              <p class="mt-3">
                Already registered?
  ...
  methods: {
          ...
          .then((res) => {
            if (res) {
              this.$notification.success(
                "Your have been successfully registered, Check your email and activate your account",
                { infiniteTimer: false }
              );
          ...
          .catch((err) => {
            if (err == 400) {
              this.$notification.error(
                "Email has already been registered please check and try again",
                { infiniteTimer: false }
              );
            } else if (err == 500) {
              this.$notification.error(
                "There is a problem from our side. Please try again later",
                { infiniteTimer: false }
              );
            } else if (err == 403) {
              this.$notification.error(
    ...
    valid() {
      if (this.password != this.confirm_password) {
        ...
        this.$notification.error("Passwords is too weak", {
          infiniteTimer: false,
        });
        return false;
      } else {
...
.bg_img {
  background: linear-gradient(to bottom, #ed2f527a, #ed2f527a),
    url("../../assets/images/userdisplay.png") no-repeat;
